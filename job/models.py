...
class JobData(models.Model):
    ...
    href = models.TextField('职位链接', blank=True, null=True)  # 职位链接，使用TextField支持更长的URL
...
class UserList(models.Model):
    user_id = models.IntegerField('用户ID', primary_key=True)  # 用户ID，手动设置的主键
    user_name = models.CharField('用户名称', max_length=255, blank=True, null=True)  # 用户名称(昵称)
    user_account = models.CharField('用户账号', max_length=255, blank=True, null=True)  # 用户账号
